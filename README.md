...
- **Configuration as code** — monitors, alerts, channels, maintenance and runtime auth settings live in one `config.yaml`; CI provisions the D1 database and notification queue automatically
...
- A Cloudflare Access application for the private page, set up in the Cloudflare dashboard
...
`deploy:prod` runs the whole pipeline: tests → migration lint → **provision** (creates the D1 database and queue by name) → D1 migrations → config import → `wrangler deploy` → secrets sync. No D1 or queue IDs need to be entered by hand. Details in [DEPLOYMENT.md](docs/DEPLOYMENT.md).
...
auth: # Cloudflare Access runtime verification for the /private page
...
- Generic webhooks POST JSON with `monitor`, `incidentId`, `status`, `eventType`, `count`, optional `errorMessage`, `message`, `cronTimestamp` and `timestamp`. Use `headers` for auth, for example `Authorization: Bearer ${DEMO_WEBHOOK_TOKEN}`.
- For a quick notification demo, create a temporary receiver at [Webhook.site](https://webhook.site/) or [Beeceptor](https://beeceptor.com/), set `DEMO_WEBHOOK_URL`, and enable the `Demo Webhook` channel in `config.yaml`.
...
- Secrets never go into YAML or D1 — use `${VAR}` placeholders, resolved from the Worker's environment at runtime. `scripts/sync-secrets.ts` discovers referenced placeholders during deploy, so a demo webhook URL can be provided as `DEMO_WEBHOOK_URL` in CI secrets or via `npx wrangler secret put DEMO_WEBHOOK_URL`.
- Cloudflare Access is configured manually in the dashboard. **Scope the Access application to the `/private` path** (Destination `<your-host>/private`), not the bare hostname — otherwise Access walls off the whole site, public page included. Set `CLOUDFLARE_ACCESS_TEAM_NAME` and `CLOUDFLARE_ACCESS_AUD` as runtime secrets so the Worker can verify Access JWTs for `/private`.
...
| ------------------------------ | ----------- | --------------------------------------------------------------------------------------------------------------- |
| `CLOUDFLARE_API_TOKEN`         | deploy-time | Used by the deploy scripts and CI (provision, config import). Never reaches the Worker.                         |
| `CLOUDFLARE_ACCOUNT_ID`        | deploy-time | Same; also injected into the generated `wrangler.jsonc` vars for the usage block.                               |
| `CLOUDFLARE_ACCESS_TEAM_NAME`        | runtime     | Cloudflare Access team subdomain for JWT verification.                                                          |
| `CLOUDFLARE_ACCESS_AUD`                | runtime     | Cloudflare Access application AUD tag for JWT verification.                                                     |
...
| `DEMO_WEBHOOK_URL`, …          | runtime     | Custom notification variables — one per `${VAR}` placeholder in `config.yaml` notification channels, same name. |

Recommended Cloudflare API token permissions:

| Token                          | Required permissions                                                                                                                                                               | Notes                                                                                                                        |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `CLOUDFLARE_API_TOKEN`         | Workers Scripts:Edit, D1:Edit, Queues:Edit                                                                                                                                         | Used only by deploy/provision scripts and CI. Add Workers Routes:Edit and Zone:Read if deploying a custom domain route.      |
...
| Notification secrets           | None in Cloudflare                                                                                                                                                                 | Values like `DEMO_WEBHOOK_URL`, `MATTERMOST_WEBHOOK_URL` or `TELEGRAM_BOT_TOKEN` are third-party credentials, stored as Worker secrets and resolved at send time. |
...
2. **Manual** — upload each secret once by hand; it persists across deployments:
   ...
   npx wrangler secret put DEMO_WEBHOOK_URL
   npx wrangler secret put DEMO_WEBHOOK_TOKEN
   ```

Adding a new placeholder such as `url: ${MY_NEW_HOOK}` or `bot_token: ${TELEGRAM_BOT_TOKEN}` means adding that secret the same way (CI secret or `wrangler secret put`) — the Worker substitutes it at runtime, so the value never appears in YAML, D1 or git.
